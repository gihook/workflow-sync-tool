...
const sourceHost = "https://tbms-dev.rationaletech.com";
const sourceCookie =
...
const targetHost = "http://localhost:5000";
const targetCookie =
...
async function main() {
  const sourceHelper = new WorkflowTemplateHelper(sourceHost, sourceCookie);
  const sourceWorkflowTemplates = await sourceHelper.getAllWorkflowTemplates();
  const sourceDict = createDictionary(sourceWorkflowTemplates);

  const targetHelper = new WorkflowTemplateHelper(targetHost, targetCookie);
  const targetWorkflowTemplates = await targetHelper.getAllWorkflowTemplates();
  const targetDict = createDictionary(targetWorkflowTemplates);

  const result = {
    sourceTemplateIds: Object.keys(sourceDict),
    updatedTemplates: [],
    createdTemplates: [],
    templateErrors: [],
  ...
  const numberOfOperations = result.sourceTemplateIds.length + 1;
  let operationCount = 1;

  for (const templateId in sourceDict) {
    logProgress(operationCount, numberOfOperations);

    try {
      const targetId = targetDict[templateId]?.id;
      const yaml = await sourceHelper.getYaml(sourceDict[templateId].id);

      if (targetId) {
        await targetHelper.updateTemplate(targetId, yaml);
        result.updatedTemplates.push(templateId);
      } else {
        await targetHelper.createTemplate(templateId, yaml);
  ...
  const sourceTemplateMatchers = await sourceHelper.getTemplateMatchers();

  try {
    await targetHelper.updateTemplateMatchers(sourceTemplateMatchers);
...
function logProgress(index, totalCount) {
  const percentage = calculateProgres(index, totalCount);
  console.log(`Completion: ${percentage}%`);
}

function calculateProgres(index, totalCount) {
